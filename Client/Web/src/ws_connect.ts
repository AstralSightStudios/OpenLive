...
import { show_error_dialog, show_info_dialog, close_login_dialog, refresh_ui_userinfo } from './universal_commands'
...
export function StartWSConnect() {
    ...
    (window as any).ws_connect.addEventListener('message', function (event: any) {
        ...
        if (data.resp_action === "QUIET_LOGIN_REQUEST_RESP") {
            if (data.resp.status) {
                (window as any).logged = true;
                (window as any).uid = data.resp.data.account_object.UID;
                (window as any).username = data.resp.data.account_object.USERNAME;
                (window as any).display_name = data.resp.data.account_object.DISPLAY_NAME;
                (window as any).token = getCookie("latest_token")
                refresh_ui_userinfo()
            }
            else {
        ...
        if (data.resp_action === "LOGIN_REQUEST_RESP") {
            if (data.resp.status) {
                ...
                (window as any).username = data.resp.data.account_object.USERNAME;
                (window as any).display_name = data.resp.data.account_object.DISPLAY_NAME;
                (window as any).token = data.resp.data.token
                setCookie("latest_token", data.resp.data.token, 30)
                close_login_dialog()
                show_info_dialog("登录成功","欢迎回来，" + (window as any).display_name + "！")
                refresh_ui_userinfo()
            }
            else {
